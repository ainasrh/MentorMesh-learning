...
def process_message(ch, method, properties, body):
    try:
        ...
        try:
            ...
            logger.info(f"Returning {len(course_data)} courses")
        except Course.DoesNotExist:
            logger.warning(f"error  when fecthing all courses")
...
def start_consumer():
    try:
        ...
        channel.queue_declare(queue="allCourse_que", durable=True)

        logger.info(" Connected to RabbitMQ and waiting for messages...")
        channel.basic_consume(queue="allCourse_que", on_message_callback=process_message)
